  ...
  static MonthlySummary fromTransactions(List<dynamic> transactions) {
    ...
    for (final txn in transactions) {
      if (txn is Map<String, dynamic>) {
        final amount = (double.tryParse(txn['Money']?.toString() ?? '0') ?? 0).abs();
        totalSpent += amount;

        final timeStr = txn['Time']?.toString() ?? '';
        final match = RegExp(r'^(\d{4})/(\d{1,2})/(\d{1,2})').firstMatch(timeStr);
        if (match != null) {
          String pad(String v) => v.padLeft(2, '0');
          final day = '${match.group(1)}-${pad(match.group(2)!)}-${pad(match.group(3)!)}';
    ...
    final avgPerTransaction = transactions.isNotEmpty ? totalSpent / transactions.length : 0.0;
    final maxDailySpent = dailySpending.values.isEmpty ? 0.0 : dailySpending.values.reduce((a, b) => a > b ? a : b);
